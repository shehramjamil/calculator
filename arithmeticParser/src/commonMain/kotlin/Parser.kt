...
open class Parser<E : Any>(private val composer: ExpressionComposer<E>) {
    ...
    internal tailrec fun EndedWithExpression<E>.reduced(): EndedWithExpression<E> =
        when (this.prefix) {
...
class PartialParser<E : Any, PE : Any>(
    ...
    data class Result<E : Any, PE : Any>(
        val expression: E?,
        val partialExpression: PE,
...
internal class Tokenizer(private val expression: String) {
    ...
    fun tryReadBinaryOperator(): BinaryOperator? =
        BinaryOperator.values().firstOrNull { tryRead(it.sign) }
